...
public:
  ...
  nlohmann::json drill_param_spec() const override {
    ...
    drills["interval"] = interval_schema().to_json();
    drills["note"] = note_schema().to_json();
    drills["melody"] = melody_schema().to_json();
    drills["chord"] = chord_schema().to_json();
    spec["drills"] = std::move(drills);
    return spec;
  }
